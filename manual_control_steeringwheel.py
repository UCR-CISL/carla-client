...
class CameraManager(object):
    def __init__(self, parent_actor, hud):
        ...
        self._camera_transforms = [
            carla.Transform(carla.Location(x=0.0, y=-0.3, z=1.3)),  # First person
            carla.Transform(carla.Location(x=0.0, y=-0.3, z=1.3), carla.Rotation(yaw=-60)),  # Left side view
            carla.Transform(carla.Location(x=0.0, y=-0.3, z=1.3), carla.Rotation(yaw=60)),  # Right side view
        ...
        for mirror_info in self.sensors_side_mirrors_info:
            ...
            bp.set_attribute('image_size_x', str(int(hud.dim[0] / 4)))
            bp.set_attribute('image_size_y', str(int(hud.dim[1] / 4)))
            bp.set_attribute('fov', str(45.0))
            mirror_info.append(bp)

        for item in self.reverse_mirror_info:
            bp = bp_library.find(item[0])
            bp.set_attribute('image_size_x', str(int(hud.dim[0] / 2)))
            bp.set_attribute('image_size_y', str(int(hud.dim[1] / 4)))
            item.append(bp)
    ...
    def render(self, display):
        ...
        if self.surface_side_mirrors[0] is not None:
            display.blit(self.surface_side_mirrors[0], (500, int(3 * self.hud.dim[1] / 4)))
        if self.surface_side_mirrors[1] is not None:
            display.blit(self.surface_side_mirrors[1],
                         (int(3 * self.hud.dim[0] / 4) - 200, int(3 * self.hud.dim[1] / 4)))
        if self.surface_reverse is not None:
            display.blit(self.surface_reverse, (int(self.hud.dim[0] / 4), 0))
